...
class CleepsRepositoryImpl(
    ...
    override suspend fun getCleeps(): List<Cleep> = api.getCleeps().cleeps.map(CleepDto::toDomain)

    override suspend fun createCleep(content: String): Cleep =
        api.createCleep(CreateCleepRequest(content = content)).toDomain()
